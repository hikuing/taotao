...
<table class="easyui-datagrid" id="itemParamList" title="商品列表" 
    ...
    <thead>
        <tr>
        	...
        	<th data-options="field:'itemCatName',width:100,formatter:formatCid">商品类目</th>
...
<script>
	...
	//格式化商品类别
    function formatCid(val,row,index) {
    	//alert(JSON.stringify(row['cid']));
    	//alert(index);
    	var id = "ccname" + index;
    	 $.ajax({
			type:"get",
			url:"/rest/item/cat/name",
			data:{"id":row.itemCatId},
			dataType:"json",
			statusCode:{
				200:function(_data) {
					$("#"+ id ).html(_data.name);
